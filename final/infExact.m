...
[n, D] = size(x);
K = cov{1}(cov{2:end}, hyp.cov, x);                      % evaluate covariance matrix
m = mean{1}(mean{2:end}, hyp.mean, x);                          % evaluate mean vector
...
if nargout>1                               % do we want the marginal likelihood?
  ...
  if nargout>2                                         % do we want derivatives?
    ...
    for i = 1:numel(hyp.cov)
      dnlZ.cov(i) = sum(sum(Q.*cov{1}(cov{2:end}, hyp.cov, x, [], i)))/2;
    end
    dnlZ.lik = sn2*trace(Q);
    for i = 1:numel(hyp.mean), 
      dnlZ.mean(i) = -mean{1}(mean{2:end}, hyp.mean, x, i)'*alpha;
    end
  end
end
