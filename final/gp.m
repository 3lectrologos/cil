...
if nargin==7                                     % if no test cases are provided
  ...
  if numel(L)==0                      % in case L is not provided, we compute it
    K = cov{1}(cov{2:end}, hyp.cov, x(nz,:));
  ...
  while nact<ns
    ...
    kss = cov{1}(cov{2:end}, hyp.cov, xs(id,:), 'diag');              % self-variance
    Ks  = cov{1}(cov{2:end}, hyp.cov, x(nz,:), xs(id,:));         % cross-covariances
    ms = mean{1}(mean{2:end}, hyp.mean, xs(id,:));
